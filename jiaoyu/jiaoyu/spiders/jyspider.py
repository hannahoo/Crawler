...
#        f=open("chi1.txt")
#        content=f.read()#one line html
#        
#        point_id=Selector(text=content).xpath('//span//a/@href').re("\d+")
#        #remove illegal int
#
#        point_id=list(set(point_id))

        for x in range(1,697+1):
            yield self.request_page(int(x),1)

    def request_page(self,pids,page):
        request_url='http://www.mofangge.com/qlist/wuli/'
        if page== 1:
            params=str(pids)
        else:
            params=str(pids)+'/'+str(page)
